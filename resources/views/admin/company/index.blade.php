...
@section('title',_trans('Companies'))
@section('content')
    <div class="container-fluid">
        <div class="page-title">
            <div class="row">
                <div class="col-6 col-sm-6">
                    <h3>{{ _trans('Companies') }}</h3>
                </div>
                @can('Company add')
        ...
        <div class="row">
            <div class="col-12 col-sm-6">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">{{ _trans('Companies') }}</li>
                </ol>
            </div>
            ...
            <div class="col-xl-12 col-md-12 ">
                ...
                <form action="{{ url()->current() }}" method="GET">
                    <div class="left-side-header">
                        <div class="row justify-content-between align-items-center">
                            ...
                            <div class="col-md-4">
                                <p class="mb-0 font-sm d-flex align-items-center justify-content-end">{{ _trans('Count Companies') }} :
                                    <span class="d-block font-md text-danger">({{ $companies->total() }})</span>
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="table-responsive custom-scrollbar p-t-30">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <th><span>{{ _trans('governorate name')}}</span></th>
                        ...
                        @foreach($companies as $key => $row)
                            <tr>
                                <td class="text-main">
                                    <a href="{{ route('admin.company.edit',$row->id) }}">Company#{{ $row->id }}</a>
                                </td>
                                <td>{{ $row->created_at->format('d-m-Y') }}</td>
                                <td>
                                    <a href="{{ route('admin.company.edit',$row->id) }}">{{ $row->translateOrDefault(locale())?->name }}</a>
                                </td>
                                <td>{{ $row->user?->name }}</td>
                                ...
                                <td>
                                    @can('Company edit')
                                        <a href="{{ route('admin.company.edit',$row->id) }}" class="btn btn-primary">
                ...
                {{ $companies->appends(request()->query())->links('layouts.partials.pagination') }}
...
@include('layouts.ajax.update-status',['class' => 'status','route' => route('admin.company.update-status')])
